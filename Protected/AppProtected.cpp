...
#include "AppProtected.hpp"

namespace {
  static const std::string c_sAppTitle(        "Option Protected" );
  static const std::string c_sAppNamePrefix(   "Protected" );
...
IMPLEMENT_APP(AppProtected)

bool AppProtected::OnInit() {
  ...
  m_pFrameMain->Bind( wxEVT_CLOSE_WINDOW, &AppProtected::OnClose, this );  // start close of windows and controls

  FrameMain::vpItems_t vItems;
  using mi = FrameMain::structMenuItem;  // vxWidgets takes ownership of the objects
  vItems.push_back( new mi( "Close, Done", MakeDelegate( this, &AppProtected::HandleMenuActionCloseAndDone ) ) );
  vItems.push_back( new mi( "Save Values", MakeDelegate( this, &AppProtected::HandleMenuActionSaveValues ) ) );
  m_pFrameMain->AddDynamicMenu( "Actions", vItems );

  vItems.clear();
  vItems.push_back( new mi( "Stop", MakeDelegate( this, &AppProtected::HandleMenuActionStopChart ) ) );
  vItems.push_back( new mi( "Start", MakeDelegate( this, &AppProtected::HandleMenuActionStartChart ) ) );
...
void AppProtected::HandleMenuActionCloseAndDone() {
  std::cout << "Closing & Done" << std::endl;
  m_pStrategy->CloseAndDone();
}

void AppProtected::HandleMenuActionSaveValues() {
...
void AppProtected::HandleMenuActionStopChart() {
  m_pWinChartView->SetChartDataView( nullptr );
}

void AppProtected::HandleMenuActionStartChart() {
  m_pWinChartView->SetChartDataView( &m_ChartDataView );
}

void AppProtected::ConstructLiveInstrument() {
...
void AppProtected::ConstructSimInstrument() {
  ...
  FrameMain::vpItems_t vItems;
  using mi = FrameMain::structMenuItem;  // vxWidgets takes ownership of the objects
  vItems.push_back( new mi( "Start", MakeDelegate( this, &AppProtected::HandleMenuActionSimStart ) ) );
  vItems.push_back( new mi( "Stop",  MakeDelegate( this, &AppProtected::HandleMenuActionSimStop ) ) );
  vItems.push_back( new mi( "Stats",  MakeDelegate( this, &AppProtected::HandleMenuActionSimEmitStats ) ) );
...
void AppProtected::HandleMenuActionSimStart() {
  CallAfter(
    [this](){
      m_sim->Run();
    }
...
void AppProtected::HandleMenuActionSimStop() {
  CallAfter(
    [this](){
      m_sim->Stop();
    }
...
void AppProtected::HandleMenuActionSimEmitStats() {
  CallAfter(
    [this](){
      std::stringstream ss;
      m_sim->EmitStats( ss );
...
void AppProtected::OnClose( wxCloseEvent& event ) {
...
void AppProtected::OnData1Connected( int ) {
  //m_bData1Connected = true;
  ConfirmProviders();
}

void AppProtected::OnData2Connected( int ) {
  //m_bData2Connected = true;
  // Data2 Connection not used
}

void AppProtected::OnExecConnected( int ) {
  //m_bExecConnected = true;
  ConfirmProviders();
}

void AppProtected::OnData1Disconnected( int ) {
  //m_bData1Connected = false;
}

void AppProtected::OnData2Disconnected( int ) {
  //m_bData2Connected = false;
}

void AppProtected::OnExecDisconnected( int ) {
  //m_bExecConnected = false;
}

void AppProtected::ConfirmProviders() {
