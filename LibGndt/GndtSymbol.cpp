...
CGndtSymbol::CGndtSymbol( LPCSTR pszSymbolName, CGndtProvider *pProvider )
: CSymbol( pszSymbolName ), GTStock( dynamic_cast<GTSession &>( *pProvider ), pszSymbolName )
{
  GTStock::CancelChart();
  GTStock::CancelQuote();
  GTStock::CancelLevel2();

  strcpy( GTStock::m_defOrder.szAccountID, pProvider->m_pAccount32->szAccountID );
  strcpy( GTStock::m_defOrder.szStock, GTStock::m_szStock );

}

CGndtSymbol::~CGndtSymbol(void) {
}

void CGndtSymbol::StartTradeWatch() {
  GTStock::RequestChart();
}

void CGndtSymbol::StopTradeWatch() {
  GTStock::CancelChart();
}

void CGndtSymbol::StartQuoteWatch() {
  GTStock::RequestQuote();
}

void CGndtSymbol::StopQuoteWatch() {
  GTStock::CancelQuote();
}

void CGndtSymbol::StartDepthWatch() {
  GTStock::RequestLevel2();
}

void CGndtSymbol::StopDepthWatch() {
  GTStock::CancelLevel2();
}

int CGndtSymbol::OnGotQuoteLevel1(GTLevel1 *pRcd) {
  //CString s;
...
int CGndtSymbol::OnGotQuotePrint(GTPrint *pRcd) {
  //CString s;
...
int CGndtSymbol::OnGotLevel2Record(GTLevel2 *pRcd) {
...
int CGndtSymbol::OnExecMsgTrade(const GTTrade &trade) {
...
int CGndtSymbol::OnGotLevel2Clear( MMID mmid ) {
  char szMMID[10];
  CString s;
...
int CGndtSymbol::OnExecMsgErrMsg(const GTErrMsg &errmsg) {
  CString sCallbackText;
...
int CGndtSymbol::OnExecMsgOpenPosition(const GTOpenPosition &open) {
...
int CGndtSymbol::OnExecMsgPending(const GTPending &pending) {
...
int CGndtSymbol::OnExecMsgSending(const GTSending &sending) {
  CString sCallbackText;
...
int CGndtSymbol::OnExecMsgCanceling(const GTCancel &cancel) {
  CString sCallbackText;
...
int CGndtSymbol::OnExecMsgCancel(const GTCancel &cancel) {
  CString sCallbackText;
...
int CGndtSymbol::OnExecMsgReject(const GTReject &reject) {
  CString sCallbackText;
...
int CGndtSymbol::OnExecMsgRemove(const GTRemove &remove) {
  CString sCallbackText;
...
int CGndtSymbol::OnExecMsgRejectCancel(const GTRejectCancel &rejectcancel) {
  CString sCallbackText;
