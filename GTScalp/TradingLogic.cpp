...
void CTradingLogic::OnDailyBarHistoryDone( IQFeedHistory *pHistory ) {
  if ( pBars->Count() > 0 ) {
    ...
    while ( NULL != pPivot ) {
      *sz = 0;
      strcat_s( sz, 10, pPivot->m_sName.c_str() );
      strcat_s( sz, 10, "R3" );
      pTradeFrame->AppendStaticIndicator( pPivot->m_R3, sz );
      *sz = 0;
      strcat_s( sz, 10, pPivot->m_sName.c_str() );
      strcat_s( sz, 10, "R23" );
      pTradeFrame->AppendStaticIndicator( pPivot->m_R23, sz );
      *sz = 0;
      strcat_s( sz, 10, pPivot->m_sName.c_str() );
      strcat_s( sz, 10, "R2" );
      pTradeFrame->AppendStaticIndicator( pPivot->m_R2, sz );
      *sz = 0;
      strcat_s( sz, 10, pPivot->m_sName.c_str() );
      strcat_s( sz, 10, "R12" );
      pTradeFrame->AppendStaticIndicator( pPivot->m_R12, sz );
      *sz = 0;
      strcat_s( sz, 10, pPivot->m_sName.c_str() );
      strcat_s( sz, 10, "R1" );
      pTradeFrame->AppendStaticIndicator( pPivot->m_R1, sz );
      *sz = 0;
      strcat_s( sz, 10, pPivot->m_sName.c_str() );
      strcat_s( sz, 10, "PVR1" );
      pTradeFrame->AppendStaticIndicator( pPivot->m_PVR1, sz );
      *sz = 0;
      strcat_s( sz, 10, pPivot->m_sName.c_str() );
      strcat_s( sz, 10, "PV" );
      pTradeFrame->AppendStaticIndicator( pPivot->m_PV, sz );
      *sz = 0;
      strcat_s( sz, 10, pPivot->m_sName.c_str() );
      strcat_s( sz, 10, "PVS1" );
      pTradeFrame->AppendStaticIndicator( pPivot->m_PVS1, sz );
      *sz = 0;
      strcat_s( sz, 10, pPivot->m_sName.c_str() );
      strcat_s( sz, 10, "S1" );
      pTradeFrame->AppendStaticIndicator( pPivot->m_S1, sz );
      *sz = 0;
      strcat_s( sz, 10, pPivot->m_sName.c_str() );
      strcat_s( sz, 10, "S12" );
      pTradeFrame->AppendStaticIndicator( pPivot->m_S12, sz );
      *sz = 0;
      strcat_s( sz, 10, pPivot->m_sName.c_str() );
      strcat_s( sz, 10, "S2" );
      pTradeFrame->AppendStaticIndicator( pPivot->m_S2, sz );
      *sz = 0;
      strcat_s( sz, 10, pPivot->m_sName.c_str() );
      strcat_s( sz, 10, "S23" );
      pTradeFrame->AppendStaticIndicator( pPivot->m_S23, sz );
      *sz = 0;
      strcat_s( sz, 10, pPivot->m_sName.c_str() );
      strcat_s( sz, 10, "S3" );
      pTradeFrame->AppendStaticIndicator( pPivot->m_S3, sz );
