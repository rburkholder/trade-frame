...
private: BarLayer* addVolBars2(XYChart* c, int height, int upColor, int downColor, int flatColor
           )
         {
           ...
           BarLayer *upLayer = addBarIndicator2(c, ArrayMath(m_volData).selectGTZ(closeChange),
             upColor, "");
           BarLayer *dnLayer = addBarIndicator2(c, ArrayMath(m_volData).selectLTZ(closeChange),
             downColor, "");
           BarLayer *flatLayer = addBarIndicator2(c, ArrayMath(m_volData).selectEQZ(closeChange),
             flatColor, "");

           double maxVol = ArrayMath(m_volData).maxValue();
           if (c == m_mainChart) {
           ...
           a->setLabelFormat(volFormat.c_str());

           int i = lastIndex(m_volData);
           std::string label = "Vol";
           if (i >= 0) {
             sprintf(buffer, "Vol: %s%s", formatValue(m_volData[i], m_generalFormat.c_str()), m_volUnit.c_str());
             label = buffer;
           }

           BarLayer *selectedLayer = 0;
           ...
           selectedLayer->getDataSet(0)->setDataName(label.c_str());
...
public: XYChart* addIndicator(int height)
        {
          ...
          if (m_currentChart != 0) {
            ...
            m_currentChart->xAxis()->setColors(Chart::LineColor, Chart::Transparent);
          ...
          ret->setPlotArea(m_leftMargin, m_topMargin, m_totalWidth - m_leftMargin - m_rightMargin,
            height, m_plotAreaBgColor)->setGridColor(m_majorHGridColor, m_majorVGridColor,
            m_minorHGridColor, m_minorVGridColor);
          ...
          box->setMargin(5, 0, 2, 0);
          box->setSize(m_totalWidth - m_leftMargin - m_rightMargin, 0);
...
public: XYChart* addChaikinOscillator(int height, int color)
        {
          ...
          ArrayMath expAvg10 = ArrayMath(accdist).expAvg(2.0 / (11));
          return addLineIndicator(height, ArrayMath(accdist).expAvg(2.0 / (4)).sub(expAvg10), color,
            "Chaikin Oscillator");
        }
...
public: XYChart* addPerformance(int height, int color)
        {
          int firstCloseIndex = m_extraPoints;
          while (firstCloseIndex < m_closeData.len) {
            if ((m_closeData[firstCloseIndex] != Chart::NoValue) && (m_closeData[firstCloseIndex] !=
              0)) {
                break;
            }
          ...
          if (firstCloseIndex < m_closeData.len) {
            return addLineIndicator(height, ArrayMath(m_closeData).div(m_closeData[firstCloseIndex]
            ).sub(1).mul(100), color, "Performance");
          } else {
            //chart is empty !!!
...
private: ArrayMath computeRSI(int period)
         {
           ...
           ArrayMath absChange = ArrayMath(m_closeData).delta().abs().movAvg(period);
           return ArrayMath(m_closeData).delta().selectGTZ().movAvg(period).financeDiv(absChange, 0.5
             ).mul(100);
         }
...
public: XYChart* addTRIX(int height, int period, int color)
        {
          ...
          return addLineIndicator(height, ArrayMath(m_closeData).expAvg(f).expAvg(f).expAvg(f).rate(),
            color, buffer);
        }
